...
const setupMessage=document.querySelector("#setup-message"),authSection=document.querySelector("#auth-section"),appSection=document.querySelector("#app-section"),authForm=document.querySelector("#auth-form"),emailInput=document.querySelector("#email"),passwordInput=document.querySelector("#password"),loginButton=document.querySelector("#login-button"),signupButton=document.querySelector("#signup-button"),logoutButton=document.querySelector("#logout-button"),authMessage=document.querySelector("#auth-message"),userEmail=document.querySelector("#user-email"),form=document.querySelector("#water-form"),dateInput=document.querySelector("#date"),amountInput=document.querySelector("#amount"),list=document.querySelector("#record-list"),empty=document.querySelector("#empty"),average=document.querySelector("#average"),count=document.querySelector("#count"),message=document.querySelector("#message"),chart=document.querySelector("#water-chart");
...
form.addEventListener("submit",async function(event){
  ...
  try{await ensureSession();const response=await apiFetch("/rest/v1/water_records",{method:"POST",headers:{"Content-Type":"application/json","Prefer":"return=representation"},body:JSON.stringify({date:dateInput.value,amount:amount,user_id:session.user.id})});const saved=await response.json();records.push(saved[0]);render();amountInput.value="";amountInput.focus();showMessage("記録を保存しました。",false)}catch(error){handleDataError(error,"記録を保存できませんでした。")}finally{setFormBusy(false)}
});
list.addEventListener("click",async function(event){
  const button=event.target.closest("button[data-id]");if(!button)return;button.disabled=true;
  try{await ensureSession();await apiFetch("/rest/v1/water_records?id=eq."+encodeURIComponent(button.dataset.id),{method:"DELETE"});records=records.filter(function(record){return String(record.id)!==button.dataset.id});render();showMessage("記録を削除しました。",false)}catch(error){button.disabled=false;handleDataError(error,"記録を削除できませんでした。")}
});
...
async function signUp(){
  ...
  try{const response=await fetch(cleanUrl()+"/auth/v1/signup",{method:"POST",headers:{apikey:SUPABASE_PUBLISHABLE_KEY,"Content-Type":"application/json"},body:JSON.stringify({email:emailInput.value.trim(),password:passwordInput.value})});const data=await readResponse(response);if(data.access_token){setSession(data);await showApp()}else{showAuthMessage("確認メールを送信しました。メールを確認してからログインしてください。",false)}}catch(error){showAuthMessage(authErrorMessage(error),true)}finally{setAuthBusy(false)}
}
...
async function showApp(){authSection.hidden=true;setupMessage.hidden=true;appSection.hidden=false;userEmail.textContent=session.user.email||"ログイン中";showMessage("記録を読み込んでいます…",false);await fetchRecords();showMessage("",false)}
...
async function fetchRecords(){await ensureSession();const response=await apiFetch("/rest/v1/water_records?select=id,created_at,date,amount,user_id&order=date.desc,created_at.desc");records=await response.json();render()}
...
async function ensureSession(){
  ...
  if(session.expires_at&&Date.now()<session.expires_at*1000-60000)return;
...
async function readResponse(response){let data={};try{data=await response.json()}catch(error){}if(!response.ok){const apiError=new Error(data.msg||data.message||data.error_description||data.error||"request_failed");apiError.status=response.status;throw apiError}return data}
...
function authErrorMessage(error){const text=String(error.message).toLowerCase();if(text.includes("invalid login credentials"))return"メールアドレスまたはパスワードが違います。";if(text.includes("email not confirmed"))return"確認メール内のリンクを開いてからログインしてください。";if(text.includes("already registered")||text.includes("already been registered"))return"このメールアドレスは登録済みです。ログインしてください。";if(text.includes("password"))return"パスワードは6文字以上で設定してください。";if(text.includes("failed to fetch"))return"Supabaseに接続できません。URLと通信環境を確認してください。";return"認証に失敗しました。入力内容を確認して、もう一度お試しください。"}
